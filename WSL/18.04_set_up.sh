...
select yn in "Yes" "No"; do
	case $yn in
		Yes )
			...
			curl -fLo ~/.tmux.conf https://raw.githubusercontent.com/msaio/hf/master/U18.04/.tmux.conf
			curl -fLo ~/.tmux.conf.local https://raw.githubusercontent.com/msaio/hf/master/U18.04/.tmux.conf.local
			break
			;;
		No  )
			break
