...
set_up_ip () {
	...
	export GLOBAL_IP=$(dig +short myip.opendns.com @resolver1.opendns.com) # sometimes shit happens
...
update_wsl_setup () {
	...
	#For .bashrc
	# Get start line
	ST=$(echo $(grep -n "UserOption" ~/.bashrc | cut -f1 -d:) | awk '{print $2;}')
	# Get end line
	EN=$(wc -l ~/.bashrc | awk '{ print $1 }')
	sed -e "$ST,$EN d" ~/.bashrc > $HOME/hf/U20.04/bash/.bashrc
	...
	case $OS_VERSION in 
		...
		18.04 )
			# Update bash config
			[ -d "$HOME/hf" ] \
			&&  sed -e "$ST,$EN d" ~/.bashrc > $HOME/hf/U18.04/bash/.bashrc \
			|| git clone https://github.com/msaio/hf.git $HOME/hf \
			&&  sed -e "$ST,$EN d" ~/.bashrc > $HOME/hf/U18.04/bash/.bashrc
			# Update nvim config
			[ -d "$HOME/hf" ] \
